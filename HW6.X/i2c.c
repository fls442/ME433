...
unsigned char MCPwrite = WRITE, MCPread = READ;
...
// Below are the functions I wrote for HW6
void i2cWrite(unsigned char add, unsigned char reg, unsigned char val){
    i2c_master_start();         // start bit 
    i2c_master_send(add);       // chip address
    i2c_master_send(reg);       // send chip register
    i2c_master_send(val);       // send register value
    i2c_master_stop();    
}
 ...
 unsigned char i2cRead(unsigned char addW, unsigned char addR, unsigned char reg){
    ...
    i2c_master_start();         // start bit
	i2c_master_send(addW);      // send device writing address
	i2c_master_send(reg);       // send register we want to read
...
void GP7on(void){
    i2cWrite(MCPwrite, 0x0A, 0b10000000);
}
